...
def _make_phase_from_offline_json(path: str | Path, joint_names: list[str]) -> ExcitationPhase:
    ...
    for point_payload in points:
        point = JointTrajectoryPoint()
        point.positions = [float(value) for value in point_payload["positions"]]
        point.velocities = [float(value) for value in point_payload.get("velocities", [0.0] * len(joint_names))]
        point.accelerations = [float(value) for value in point_payload.get("accelerations", [0.0] * len(joint_names))]
        point.time_from_start = _msg_duration(float(point_payload["time"]))
        trajectory.points.append(point)
...
def main() -> int:
    ...
    if list(args.joints) != FRANKA_JOINTS:
        ...
        return 2
    if not args.trajectory_json and not args.urdf_path:
