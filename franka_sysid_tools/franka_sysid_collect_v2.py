...
def write_manifest(path: Path, phases: list[ExcitationPhase], args: argparse.Namespace) -> None:
    manifest = {
        ...
        "notes": [
            ...
            "feedback.effort semantics depend on --torque-source: 'joint-states' forwards JointState.effort "
            "(stack-dependent, declared torque_semantics: external); 'franka-robot-state' publishes tau_J "
            "from the franka_robot_state_broadcaster (link-side, gravity-inclusive) and records the raw topic.",
        ],
